...
| 구현 함수 | **video** [[VA-MS-001#VideoService.list_inbox]] · `VideoService.info_of` 로컬 갈래(스텁 해제) · **job** [[VA-MS-002#JobService.retry]] · [[VA-MS-002#JobService.plan_chunks]] · [[VA-MS-002#JobService.mark_chunk]] · `JobService.remaining_sec` 조각 갈래 · [[VA-MS-002#pipeline.resume]] · [[VA-MS-002#pipeline.transcribe_stage]] · [[VA-MS-002#pipeline.reason_of]](실패 이유 한 줄 — B1 코드 리뷰에서 넘어온 미결) · `pipeline.run`의 download(음성) · extract · transcribe 갈래(로컬 음성은 받아쓰기 단계가 mp3로 바꾼다) · **analysis** `AnalysisService.generate_chapters`의 파트 갈래 · `generate_summary`의 구간 처리 갈래 · **adapters** [[VA-MS-006#media_probe.probe]] · [[VA-MS-006#audio_source.download_audio]] · [[VA-MS-006#audio_source.extract_audio]] · [[VA-MS-006#audio_split.split]] · [[VA-MS-006#stt_openai.transcribe]] — 새 함수 12개 + 갈래 해제 5곳 |
| API | [[VA-API-001#GET/api/inbox]] · [[VA-API-001#POST/api/videos/{id}/job/retry]] · `POST /api/videos`의 `local` 갈래 · `GET …/job`의 `chunks` |
| 화면 | [[VA-UI-002#UI-1]] 「내 파일」 카드(inbox 목록 · 선택 · 빈 안내) · 실패 행 · [[VA-UI-002#UI-2]] 받아쓰기 필요 판(조각 수 · 동시 수 · 줄별 비용, YouTube 자막 없음 · 로컬 음성 변형 — B1의 '아직 지원하지 않음' 판을 바꾼다) · [[VA-UI-002#UI-3]] 받아쓰기 중(조각 격자 · 범례 · 남은 시간) · 실패 상태(실패 알림 · 다시 시도 · 목록으로 — 제목은 실패 종류로, 본문의 '왜'는 `reason_of`의 한국어 한 줄. [[VA-UI-002#UI-3]] 규칙) · 음성을 조각으로 나누는 중 · 받아쓰기 아닌 단계 실패 · [[VA-UI-002#UI-4]] 파트 카드(펴고 접기) · `h:mm:ss` 표기 · 받아쓰기 메타 · 로컬 파일 등록 실패는 시작 불가 판에 서버가 준 이유 그대로(종류별 문구는 B5) |
| 테스트 | 구현 함수의 테스트 관점 전부 · 가짜 ffmpeg · ffprobe(pytest는 infra를 가짜로, E2E는 `e2e/fake-ffmpeg.mjs` — inbox 파일 이름으로 길이 · 음성, 고정 무음, 길이만 담은 조각 파일) · 가짜 STT(조각 하나를 세 번 실패시키는 갈래) · **E2E S2**: inbox 파일 고르기 → 받아쓰기 필요 판(15조각 · 3동시 · $0.9) → 받아쓰기 격자가 늘어남 → 파트로 묶인 챕터 · **E2E S6 4번**: 8번 조각 실패(150분 파일은 15조각이라 16번이 없다) → 실패 알림 → 다시 시도 → 끝난 조각은 다시 보내지 않고 이어감(가짜 STT의 받은 조각 기록) · 다른 영상이 도는 동안 다시 시도 → 대기열 끝에서 기다렸다 이어감 · 받아쓰기 도중 서버 재시작 → `failed`, 다시 시도로 이어감(Playwright가 띄운 api는 죽였다 살리기 어려워 pytest가 lifespan을 두 번 돌려 본다 — B1과 같다) |
...
| 선행 | B1 |
| 완료 | 2026-09-23(KST) · 브랜치 `feat/card-b2` → `dev` squash `968329c`(PR #3, 커밋 32개 — 함수 하나 = 커밋 하나, 단계마다 그 함수의 테스트가 통과한 채로, 코드 리뷰 반영 5개 포함) · 테스트: pytest 359(api 이미지 안 361 — 진짜 ffmpeg 포함) · E2E 17(카드 A 7 · B1 넷 · B2 여섯: S2 · 로컬 음성 판과 취소 · 시작 불가 둘 · 8번 조각 실패 → 다시 시도(요청이 끊기면 5.2 아래 한 줄) · 인터넷 끊김 + 대기열에서 다시 시도 · 요약 단계 실패 → 그 단계부터) · 서버 재시작은 pytest가 실제 lifespan을 두 번 돌려 본다 · `check_code` B2 전부 일치 · `check_ui` UI-2 26/26 · UI-1 23/35 · UI-3 21/27 · UI-4 20/48 — 모자란 것은 뒤 카드 몫과 도구 한계(목록 첫 항목에만 붙인 번호 · 레이아웃이 그리는 배너 번호, B1 때 싱크독 이슈로 넘김) · 화면 확인: 사용자 지시로 에이전트가 docker + Playwright MCP로, api 이미지의 진짜 ffmpeg로 만든 150분 · 90분 영상과 25분 m4a로 S2 · 나누는 중 · 격자 · 파트 · 실패 → 다시 시도 · 로컬 음성 · 자막 없는 YouTube · 시작 불가 둘을, 리뷰 뒤 다시 시도 실패 줄을(PR #3 댓글 둘) · 사용자 결정: 남은 시간은 작업 전체가 끝날 때까지, 끝난 단계의 오차는 뒤로 넘기지 않는다(화면 확인에서 받아쓰기 뒤 '약 43초' → '약 6분'으로 거꾸로 늘던 것) · 스텁 해제: B1의 받아쓰기 쪽 전부(`transcribe_stage` · `run` 세 갈래 · `remaining_sec` 조각 갈래 · 요약 구간 · 챕터 파트 · `info_of` 로컬 · `retry` · `resume` · `GET /inbox`). 남은 스텁은 B3 · B4 몫 그대로 · 되먹임(모두 반영): [[VA-DOM-002]] v20 · v21 · [[VA-DOM-003]] v6 · [[VA-UI-001]] v7 · [[VA-UI-002]] v9 ~ v11 · [[VA-API-001]] v5 · [[VA-SEQ-001]] v5 · v6 · [[VA-MS-001]] v4 · [[VA-MS-002]] v8 ~ v11 · [[VA-MS-003]] v4 · [[VA-MS-006]] v8 · v9 · 이 문서 v8 · 코드 리뷰(`/code-review`) 10건 — 고친 것 여섯(이름 바꾼 로컬 파일의 `origin` · 첫 단계 전에 멈춘 이어하기의 조각 파일 · 조각 재시도 간격 · 실패 뒤 새 조각을 시작하지 않음(UC-S3 3a2) · inbox 목록 500 · 다시 시도 실패 표시), 남긴 것 넷: 큰 로컬 파일의 해시가 길이 확인보다 먼저(B5에서 등록 순서와 함께) · 긴 스크립트의 구간 요약 · 챕터를 차례로 부름(텍스트 모델 동시 수 규칙이 없다) · 확장자 목록 사본(어댑터 · 화면) · 이어하기 음성 결정 두 곳 |
