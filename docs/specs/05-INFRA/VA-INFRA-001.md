...
`.env`에 `OPENAI_API_KEY`. `.gitignore`에 들어간다. 시작 시 키가 없거나 무효면 분석을 막고 안내한다.
...
DB와 파일 볼륨은 호스트 디스크. 밖으로 나가는 건 음성 조각(받아쓰기)과 스크립트 텍스트(요약·Q&A)뿐.
...
| 백그라운드 작업 | **프로세스 안 asyncio 태스크** | 사용자 하나, 동시 분석 하나. 큐 불필요 | Celery + Redis | 구성 요소 둘 추가. 비목표(일괄 처리) |
...
api는 [[SYNC-STD-004]] 대신 이 프로젝트의 도메인 아키텍처 규칙(CLAUDE.md, 5단계 초기화 때 기록)을 따른다. 도메인은 6단계에서 정하지만 지금 보이는 묶음은 셋 — **video**(입력·정보·저장·목록), **analysis**(받아쓰기·요약·챕터·추천 질문 파이프라인), **chat**(Q&A). 외부 호출(yt-dlp, ffmpeg, OpenAI)은 어댑터로 격리해 모델 교체·테스트 시 대체가 되게 한다.
...
| 대상 | 방식 |
|---|---|
| 사용자 → web/api | 없음. `127.0.0.1` 바인딩 ([[#C5]]) |
| api → OpenAI | `OPENAI_API_KEY` (.env) ([[#C6]]) |
...
| 내보낸 마크다운 | `data/export/` 또는 브라우저 다운로드 | 사용자 관리 |
| API 키·DB 비밀번호 | `.env` | 커밋 금지 |
| 밖으로 나가는 것 | OpenAI: 음성 조각, 스크립트 텍스트, 질문 / YouTube: 영상 ID | 전송 후 우리 통제 밖. README와 설정 화면에 명시 ([[VA-PRD-001#N3]]) |
...
- `.env.example`을 복사해 `.env`를 만들고 키를 넣는 것이 설치의 전부 ([[#C8]])
...
- [ ] 조각 길이 — 25MB 안에서 몇 분으로 자를지 (64kbps 모노 기준 10분 ≈ 4.8MB라 여유. 10분 vs 20분). MINISPEC에서
- [ ] 받아쓰기 병렬 수 — OpenAI 요청 제한과 시간당 3분 목표 사이. 3~4 동시로 시작해 측정
- [ ] 3시간 스크립트의 Q&A 구간 선별 방식 — 챕터 필터 vs 간단 임베딩. 첫 버전은 챕터 필터로 가고 품질 보고 결정
- [ ] Next.js·FastAPI·PG 정확한 버전 — 5단계 초기화 때 당시 안정판으로 고정
- [ ] 음성 파일 형식 — YouTube는 yt-dlp가 주는 m4a/opus, 로컬은 ffmpeg로 mp3 64kbps 모노 변환. whisper-1 정확도에 영향 있는지 확인
