...
api는 이 프로젝트의 도메인 아키텍처 규칙(CLAUDE.md, 5단계 초기화 때 기록)을 따른다. 도메인은 6단계에서 정하지만 지금 보이는 묶음은 셋 — **video**(입력·정보·저장·목록), **analysis**(받아쓰기·요약·챕터·추천 질문 파이프라인), **chat**(Q&A). 외부 호출(yt-dlp, ffmpeg, OpenAI)은 어댑터로 격리해 모델 교체·테스트 시 대체가 되게 한다.
