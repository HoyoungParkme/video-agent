...
영상의 **상태**는 속성이 아니라 가장 최근 작업에서 읽는다 — 작업이 없으면 「등록만 됨」(사전 안내에서 취소한 영상. 목록에 안 보이고 다시 넣으면 처음처럼), 작업이 대기·진행 중이면 「분석 중」, 실패면 「실패」, 완료면 「분석됨」이고 분석 완료 시각은 그 작업의 끝 시각이다. 상태를 영상에 따로 두면 작업과 어긋날 수 있어 두 곳에 두지 않는다(하위 되먹임 2026-09-21). 목록([[VA-UC-001#UC-H5]])은 이 상태로 완료·진행 중·대기 중·실패를 구분해 보여준다.

#### AnalysisJob 작업

**작업**은 영상 하나에 대한 분석 시도 한 번이다. **상태**(대기 · 진행 · 실패 · 완료)와 **단계**(다운로드 → 음성 추출 → 받아쓰기 → 핵심 요약 → 챕터 → 추천 질문 중 지금 것 — 실패했으면 실패한 단계)를 따로 갖는다. 그래야 「어느 단계에서 실패했나」가 남는다. 단계는 출처에 필요한 것만이라 작업이 시작할 때 **필요한 단계들**을 정해 둔다([[VA-UC-001#UC-S6]]). 진행률, 실패 이유, 시작 전에 계산한 예상 소요 시간·비용도 여기 둔다([[VA-UC-001#UC-S1]] 5번). 동시에 도는 작업은 하나이고 나머지는 「대기」로 줄을 선다([[VA-UC-001#UC-H0]] 3b).
...
**조각**은 받아쓰기 API의 25MB 제한([[VA-INFRA-001#C2]]) 때문에 음성을 나눈 한 토막이다. 순번, 전체 음성에서의 시작 오프셋, 임시 파일 경로, 상태(대기 · 전송 중 · 완료 · 실패), 그리고 **받아쓰기 결과**(그 조각의 시각 붙은 문장들)를 갖는다. 오프셋이 있어야 조각별 시각을 전체 시각으로 되돌린다([[VA-UC-001#UC-S3]] 4번). 결과를 조각에 두는 이유는 실패 뒤 재개가 끝난 조각을 다시 보내지 않으려면 그 결과가 남아 있어야 하기 때문이다([[VA-UC-001#UC-S3]] 3a3).
...
- [x] 대화 턴의 근거 시각이 없는 경우("영상에서 다루지 않습니다") — 결정: 빈 목록. null이 아니다. 화면은 빈 목록이면 '영상에 없는 내용' 표시를 붙인다(와이어프레임 UI-4)
- [x] 영상의 상태와 분석 완료 시각을 속성으로 둘지 — 결정: 두지 않고 최근 작업에서 읽는다(3장 Video). 작업의 상태·단계 분리, 조각의 상태·결과도 같은 되먹임(API·클래스 명세·시퀀스·MINISPEC)에서 온 것
