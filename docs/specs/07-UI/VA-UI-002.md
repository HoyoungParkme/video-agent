...
- 이 화면을 열어 둔 동안 진행 중 행은 서버에서 새로 받아 6.6과 6.7을 갱신하고, 끝나면 완료 행으로, 실패하면 실패 행으로 바꾼다. 대기 중 행도 같이 새로 받아 차례({n})를 고치고, 차례가 오면 진행 중 행으로 바꾼다. 받는 주기는 MINISPEC에서 정한다 (VA-UI-001에 없음)
